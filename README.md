...
Your company wants to offer text-to-speech as an API. Developers will integrate it into real applications and expect it to work reliably. Integrate a real open-source TTS model of your choice.
...
Before your final push, run the packaging script:

```bash
./dist/package-all-sessions.sh
```

This packages your Claude Code and Codex session logs automatically. If you used other AI tools (ChatGPT, Cursor, etc.), export those conversations and include them too.
...
```bash
# 1. Clone this repo
git clone https://github.com/lumalabs/eng-take-home.git my-project
cd my-project
...
```bash
# 1. Package your AI session history
./dist/package-all-sessions.sh
...
# 3. Grant access to your reviewers (we'll provide GitHub handles in the email)
gh repo edit --add-collaborator <reviewer-handle>
```

Reply to the email thread with your repo URL.
